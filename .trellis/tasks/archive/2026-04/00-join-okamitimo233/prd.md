...
`okamitimo233` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/okamitimo233/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/okamitimo233/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee okamitimo233` to
...
python ./.trellis/scripts/task.py archive 00-join-okamitimo233
```
